...
purchase *get_next_list(purchase *head, potato new, int budget) {
...
double get_max_score(int budget, int n, double prices[], double insta_scores[]) {
    ...
    for (int i = 1; i < idx; i++) {
        ...
        purchase *next_list = get_next_list(head, potatoes[i], budget);
    ...
    // Iterate over all the purchases to find the one with the highest score
    purchase *checker = head;
    while (checker != NULL) {
        if (checker->sum_score > result) {
            result = checker->sum_score;
        }
